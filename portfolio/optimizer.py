...
from portfolio.numba_ops import (
    fast_nondominated_sort_jit,
    sbx_crossover_jit,
    gaussian_mutate_jit,
)
...
class PortfolioNSGA2Utils(NSGA2Utils):
    ...
    # -- Numba-accelerated non-dominated sorting --

    def fast_nondominated_sort(self, population):
        """Non-dominated sorting using Numba JIT-compiled inner loops."""
        ...
        objectives = np.empty((n, m))
        for i, ind in enumerate(individuals):
            for j in range(m):
                objectives[i, j] = ind.objectives[j]

        ranks = fast_nondominated_sort_jit(objectives)
