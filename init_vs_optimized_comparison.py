# plot_return_vol_init_compare.py
import math
import numpy as np
...
class ReturnVolProblem(Problem):
    """
    2-objective portfolio problem (minimization form):
      f1 = annualized volatility  (min)
      f2 = -annualized return     (min)  <=> maximize annualized return
    """
    def __init__(self, mean_returns_daily, cov_daily):
        ...
        super().__init__(
            objectives=[lambda w: 0.0],  # unused (we override calculate_objectives)
...
def lou_downselect(problem, N: int, multiplier: int, seed: int) -> Population:
    utils = NSGA2Utils(problem, num_of_individuals=N)
    large = make_population(problem, N * multiplier, seed=seed)
...
def xy_vol_ret(inds):
    # objectives are [vol_ann, -ret_ann]
...
def draw(ax, title, blue_pop, rank0, rest):
    ...
    h1 = ax.scatter(bx, by, s=6, c="tab:blue", alpha=0.20, linewidths=0, label="Random solutions")
    h2 = ax.scatter(yx, yy, s=7, c="gold", alpha=0.45, linewidths=0, label="Population")
    h3 = ax.scatter(rx, ry, s=10, c="tab:red", alpha=0.95, linewidths=0, label="Pareto rank-0")
...
def main():
    ...
    # Blue dots = random solution space (same in both panels)
    blue = make_population(problem, N, seed=1)

    # Standard init: random N
    std_pop = make_population(problem, N, seed=2)
    std_rank0, std_rest = split_rank0(std_pop)

    # Optimized init: downselect N from N*MULT
    opt_pop = lou_downselect(problem, N, MULT, seed=3)
    opt_rank0, opt_rest = split_rank0(opt_pop)

    fig, axes = plt.subplots(1, 2, figsize=(10.8, 4.2), dpi=180)

    handles = draw(axes[0], f"Standard initialization (N={N})", blue, std_rank0, std_rest)
    draw(axes[1], f"Optimized initialization (N={N}, pool={N*MULT})", blue, opt_rank0, opt_rest)

    # match limits
    x_all = np.concatenate([
        xy_vol_ret(blue.population)[0],
        xy_vol_ret(std_rest)[0], xy_vol_ret(std_rank0)[0],
        xy_vol_ret(opt_rest)[0], xy_vol_ret(opt_rank0)[0],
    ])
    y_all = np.concatenate([
        xy_vol_ret(blue.population)[1],
        xy_vol_ret(std_rest)[1], xy_vol_ret(std_rank0)[1],
        xy_vol_ret(opt_rest)[1], xy_vol_ret(opt_rank0)[1],
    ])
    ...
    fig.legend(
        ...
        ncol=3,
        frameon=False,
        bbox_to_anchor=(0.5, 0.01),  # inside the figure, near bottom
    )
